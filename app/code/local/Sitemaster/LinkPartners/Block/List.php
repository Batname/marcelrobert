...
class Sitemaster_LinkPartners_Block_List extends Sitemaster_LinkPartners_Block_Abstract
{
    ...
    protected $x23 = true;
    protected $x24 = NULL;
    protected $x25 = array(Mage_Eav_Model_Entity_Attribute::CACHE_TAG, self::CACHE_TAG);
    ...
    protected function _prepareSelectedBrandsAndFlags()
    {
        ...
        if ($x27 === NULL) {
            if ($this->_helper->getCfg('list/all_brands')) {
                ...
                $this->x23 = false;
                $this->x22 = $this->_helper->getCfg('list/brands');
            }
        } else {
            $this->x23 = false;
            $this->x22 = $x27;
    ...
    protected function _prepareCollectionCacheKey()
    {
        ...
        $this->x20 = 'brands-' . md5(implode("|", $x28));
    }
    ...
    protected function _getAllBrandsInUse()
    {
        ...
        $x2f = Mage::getResourceModel('catalog/product_collection')->addAttributeToSelect($x17)->addAttributeToFilter($x17, array('neq' => ''))->addAttributeToFilter($x17, array('notnull' => true))->addAttributeToFilter('visibility', Mage_Catalog_Model_Product_Visibility::VISIBILITY_BOTH)->addStoreFilter(Mage::app()->getStore()->getId());
    ...
    {
        if (FALSE === isset($this->x24[$x1b][$x12])) {
            $this->x24[$x1b][$x12] = $this->_formatBrandUrlKey($x12, $x1b);
        }
        return $this->x24[$x1b][$x12];
    }
}